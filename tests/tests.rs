...
fn test_generate_moves() {
    ...
    let mut engine = Engine::new();
    let res_correct = vec![
        13637, 17477, 17221, 18245, 21829, 25925, 30021, 34117, 38213, 42309, 18520, 14424, 22360, 22872,
        23128, 23384, 26712, 30808, 34904, 39000, 22375, 26215, 26727, 25975, 34167, 26999, 35191, 34439,
...
fn test_search_book() {
    ...
    engine.from_fen(fen);
    let res_correct = vec![
        33683, 34197, 34711, 35225, 35739, 38052, 33956, 29860, 25764, 13476, 41892, 42404, 42660, 42916,
        43172, 43428, 46244, 39594, 35498, 31402, 27306, 15018, 43434, 43178, 42922, 42666, 42410, 43946,
        47786, 46019, 41923, 41924, 42436, 41925, 42949, 47046, 47047, 47048, 42953, 43977, 43466, 43978,
        48075, 43979,
    ];
...
fn test_unsigned_right_shift() {
    let t = util::unsigned_right_shift(50343, 30);
    assert_eq!(t, 3);
}

#[test]
fn test_move2pos() {
    let ((src_row, src_col), (dst_row, dst_col)) = util::move2pos(34726);
    assert_eq!(src_row, 2);
    assert_eq!(src_col, 3);
    assert_eq!(dst_row, 4);
    assert_eq!(dst_col, 4);
...
fn test_pos2iccs() {
    ...
    let dst_row = 4;
    let dst_col = 4;
    assert_eq!(util::pos2iccs(src_row, src_col, dst_row, dst_col), "d2e4")
}

#[test]
fn test_move2iccs() {
    let t = util::move2iccs(22375);
    assert_eq!(t, "e6e7");
}

#[test]
fn test_iccs2move() {
    let t = util::iccs2move("d2e4");
    assert_eq!(t, 34726)
}
