...
pub const fn rank_y(sq: isize) -> isize { sq >> 4 }

pub const fn file_x(sq: isize) -> isize { sq & 15 }

pub const fn coord_xy(x: isize, y: isize) -> isize { x + (y << 4) }

pub const fn mirror_square(sq: isize) -> isize { coord_xy(14 - file_x(sq), rank_y(sq)) }

pub const fn mirror_move(mv: isize) -> isize { merge(mirror_square(src(mv)), mirror_square(dst(mv))) }

pub const fn square_forward(sq: isize, sd: isize) -> isize { sq - 16 + (sd << 5) }

pub const fn side_tag(sd: isize) -> isize { 8 + (sd << 3) }

pub const fn opp_side_tag(sd: isize) -> isize { 16 - (sd << 3) }

pub const fn src(mv: isize) -> isize { mv & 255 }

pub const fn dst(mv: isize) -> isize { mv >> 8 }

pub const fn merge(src: isize, dst: isize) -> isize { src + (dst << 8) }

pub const fn unsigned_right_shift(x: isize, y: isize) -> isize {
...
pub const fn iccs2pos(iccs: &str) -> ((usize, usize), (usize, usize)) {
...
pub const fn move2pos(mv: isize) -> ((usize, usize), (usize, usize)) {
